...
# Preparation for map
map <- tidy_income %>%
  # Sequence to create GEOID variable to join with us.map.county object
  ...
  # Limit values to only include income
...
rich_neighborhood <- neighborhood %>%
  mutate(state_code = ifelse(nchar(state_code) == 1, paste(0, state_code, sep = ""), state_code)) %>%
  mutate(county_code = ifelse(nchar(county_code) == 1, paste("00", county_code, sep = ""), county_code)) %>%
  mutate(county_code = ifelse(nchar(county_code) == 2, paste(0, county_code, sep = ""), county_code)) %>%
  mutate(GEOID = paste(state_code, county_code, sep = "")) %>%
...
rich_neighborhood <- rich_neighborhood[, 5:36]

rich_map <- map %>%
  select(GEOID, id, all) %>%
  left_join(rich_neighborhood, by = "GEOID")
...
leafmap_rich <- leafmap_rich[!is.na(leafmap_rich@data$all), ]
